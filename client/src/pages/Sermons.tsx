...
const Sermons = () => {
  ...
  return (
    <>
      ...
      <section className="py-16 bg-gray-50" id="series">
        <div className="container mx-auto px-4 lg:px-8">
          <div className="text-center mb-16">
            <motion.h2 
              className="text-3xl md:text-5xl font-montserrat font-bold text-deepPurple mb-4"
            ...
            <motion.p 
              ...
              transition={{ duration: 0.6, delay: 0.4 }}
            >
              Explore our messages organized by topical series
            </motion.p>
          </div>
            ...
            {seriesList.map((series, index) => (
              ...
              >
                <div className="relative h-48 overflow-hidden">
                  {/* Use the first sermon image from that series */}
                  <img 
                    src={extendedSermons.find(sermon => sermon.series === series)?.image}
                    alt={`${series} Series`}
                    className="w-full h-full object-cover"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent"></div>
                  <div className="absolute bottom-0 w-full p-4">
                    <h3 className="text-xl text-white font-bold">{series}</h3>
                    <p className="text-white/80 text-sm">
                      {extendedSermons.filter(sermon => sermon.series === series).length} messages
                    </p>
                  </div>
                </div>
                <div className="p-5">
                  <p className="text-gray-700 text-sm mb-4">
                    {extendedSermons.find(sermon => sermon.series === series)?.description.substring(0, 80)}...
                  </p>
                  <button 
                    onClick={() => { 
                      if (series) {
